...
<script type="text/x-template" id="v-carousel-template">
    <div class="relative m-auto flex w-full overflow-hidden shadow-[0_4px_20px_rgba(0,0,0,0.1)]">
        <!-- Slider -->
        <div 
        ...
        <!-- Navigation Arrows -->
        <button
            class="absolute left-4 top-1/2 -translate-y-1/2 hidden h-12 w-12 items-center justify-center rounded-full bg-brandBlack/80 text-gold backdrop-blur-sm transition-all duration-300 hover:bg-brandBlack hover:shadow-[0_0_20px_rgba(196,163,90,0.4)] md:flex"
            :class="{
                'cursor-not-allowed opacity-30': direction == 'ltr' && currentIndex == 0,
                'cursor-pointer hover:scale-105': direction == 'ltr' ? currentIndex > 0 : currentIndex <= 0
            }"
            role="button"
        ...
        <button
            class="absolute right-4 top-1/2 -translate-y-1/2 hidden h-12 w-12 items-center justify-center rounded-full bg-brandBlack/80 text-gold backdrop-blur-sm transition-all duration-300 hover:bg-brandBlack hover:shadow-[0_0_20px_rgba(196,163,90,0.4)] md:flex"
            :class="{
                'cursor-not-allowed opacity-30': direction == 'rtl' && currentIndex == 0,
                'cursor-pointer hover:scale-105': direction == 'rtl' ? currentIndex < 0 : currentIndex >= 0
            }"
            role="button"
        ...
        <!-- Pagination Dots -->
        <div class="absolute bottom-6 left-0 flex w-full justify-center gap-2 max-md:bottom-4 max-sm:bottom-3">
            <button
                v-for="(image, index) in images"
                class="h-2.5 w-2.5 rounded-full transition-all duration-300 max-md:h-2 max-md:w-2 max-sm:h-1.5 max-sm:w-1.5"
                :class="{ 
                    'bg-gold w-8 max-md:w-6 max-sm:w-4': index === Math.abs(currentIndex), 
                    'bg-white/50 hover:bg-white/80': index !== Math.abs(currentIndex) 
                }"
                role="button"
