...
<v-brand-showcase
    ...
    brand-name="{{ $options['brand_name'] ?? '' }}" brand-subtitle="{{ $options['brand_subtitle'] ?? '' }}"
    ...
    <div class="w-full overflow-hidden bg-white py-12 max-md:py-8">
        <div class="container mx-auto max-lg:px-8 max-md:px-4">
            ...
            <div class="flex gap-5 overflow-hidden">
                @for ($i = 0; $i < 5; $i++)
                    <div class="shimmer rounded-[24px] shrink-0 min-w-[200px] h-[320px] max-sm:min-w-[150px] max-sm:h-[240px]">
                    </div>
                @endfor
            </div>
        </div>
                ...
                <div class="relative">
                    ...
                    >
                        <a
                            ...
                            :key="category.id"
                            :href="category.slug"
                            class="group relative flex items-center justify-center transition-all duration-300 hover:-translate-y-1 shrink-0 min-w-[200px] max-w-[200px] h-[320px] max-sm:min-w-[150px] max-sm:max-w-[150px] max-sm:h-[240px]"
                        >
                            <!-- Card image -->
                            <div class="relative flex items-center justify-center h-full w-full bg-white">
                                <img
                                    :src="category.logo?.original_image_url || '{{ bagisto_asset('images/small-product-placeholder.webp') }}'"
                                    :alt="category.name"
                                    class="max-h-full max-w-full object-contain rounded-[24px] transition-transform duration-500 group-hover:scale-105"
                                    loading="lazy"
                                />
                            </div>
                        </a>
        ...
        <template v-if="isLoading">
            <div class="w-full overflow-hidden bg-white py-12 max-md:py-8">
                <div class="container mx-auto max-lg:px-8 max-md:px-4">
                    ...
                    <div class="flex gap-5 overflow-hidden">
                        <div v-for="n in 5" :key="n" class="shimmer rounded-[24px] shrink-0 min-w-[200px] h-[320px] max-sm:min-w-[150px] max-sm:h-[240px]"></div>
                    </div>
                </div>
            </div>
        </template>
