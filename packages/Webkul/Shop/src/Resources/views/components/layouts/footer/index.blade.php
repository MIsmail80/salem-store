...
<footer class="mt-9 bg-brandBlack max-sm:mt-10">
    <div
                ...
                @foreach ($customization->options as $footerLinkSection)
                    <ul class="grid gap-5 text-sm text-gold">
                        ...
                        @foreach ($footerLinkSection as $link)
                            <li>
                                <a href="{{ $link['url'] }}" class="hover:text-gold-light">
                                    {{ $link['title'] }}
                                </a>
                            </li>
                        @endforeach
        ...
        <x-shop::accordion :is-active="false"
            ...
            <x-slot:header
                class="rounded-t-lg bg-brandBlack-light font-medium text-gold max-md:p-2.5 max-sm:px-3 max-sm:py-2 max-sm:text-sm">
        ...
        @if (core()->getConfigData('customer.settings.newsletter.subscription'))
            <div class="grid gap-2.5 max-1060:w-full">
                ...
                <p class="text-xs text-gold-muted">
    ...
    <div class="flex justify-between bg-brandBlack-light px-[60px] py-3.5 max-md:justify-center max-sm:px-5">
        {!! view_render_event('bagisto.shop.layout.footer.footer_text.before') !!}

        <p class="text-sm text-gold/70 max-md:text-center">
